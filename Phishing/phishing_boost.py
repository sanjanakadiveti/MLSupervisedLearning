...
plt.figure()
title = 'Phishing AdaBoost Decision Tree: Accuracy x Num Estimators'
plt.title('\n'.join(wrap(title,60)))
...
plt.ylabel('Accuracy')
plt.savefig('Phishing_Boost_NumEstimators.png', dpi=300)
print 'plot complete'
...
print 'plotting results'
plt.figure()
plt.title('Phishing Boosted Decision Tree: Accuracy x Training Set Size')
...
plt.ylabel('Accuracy')
plt.savefig('Phishing_Boost_TrainSize.png', dpi=300)
### ---


# final confusion confusion_matrix
t = DecisionTreeClassifier(max_depth = MAX_DEPTH)
clf = AdaBoostClassifier(base_estimator=t, n_estimators=NUM_EST)
t = time.time()
